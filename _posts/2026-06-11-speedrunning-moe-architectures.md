...
*My first attempt at autoresearch and scaling laws ([codebase](https://github.com/Chang-Han-Chen/autoresearch-moe))*
{: .post-subtitle}
...
Extrapolating that fit out to F4's compute ($$3.8\times10^{19}$$ FLOPs, ~2.6× beyond the last fitted point) predicts BPB **0.728**, while the actual F4 run came in at **0.737** — about **1.2% higher** than the trend. So the held-out point lands close, but on the *worse* side: F4 slightly underperforms its own extrapolation. That's the expected direction if F4 is a touch undertrained — recall its lower-LR arm was aborted before finishing — so I read this less as "the scaling law broke" and more as "I didn't tune F4 as carefully as the smaller points." Either way, a held-out point 2.6× beyond the fitted range landing within ~1% is more than I expected from five single-seed runs.
...
*Code, research logs, and both `program.md` files: [Chang-Han-Chen/autoresearch-moe](https://github.com/Chang-Han-Chen/autoresearch-moe), adapted from [Karpathy's autoresearch](https://github.com/karpathy/autoresearch). GPUs are rented from VESSL AI and Vast.ai. Corrections and feedback very welcome — this is a learning project and I'd love to hear where I went wrong.*
